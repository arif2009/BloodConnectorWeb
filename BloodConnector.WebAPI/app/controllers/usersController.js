'use strict';
app.controller('usersController', ['usersService', '$filter', function (usersService, $filter) {
    ...
    vm.search = function () {
        ...
        if (vm.sort.sortingOrder !== '') {
            vm.filteredItems = $filter('orderBy')(vm.filteredItems, vm.sort.sortingOrder, vm.sort.reverse);
        }
        vm.currentPage = 0;
        // now group by pages
        vm.groupToPages();
